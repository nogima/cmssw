...
// xbin and ybin are coordinates (um) ínside the pixel as in the test beam, swapped wrt plane coodinates.
	    int xbin=int(round((((*i).Y()-pixel_center_y)+pixel_width_y/2.)*1.e3/5.))-1;
            int ybin=int(round((((*i).X()-pixel_center_x)+pixel_width_x/2.)*1.e3/5.))-1;
//            std::cout << "Pixel Y: " << (*i).Y()-pixel_center_y << "  Pixel X " << (*i).X()-pixel_center_x;
//            std::cout << "  pixel_center_y: " << pixel_center_y << "  pixel_center_x: " << pixel_center_x;
//            std::cout  << " pixel_row: " << pixel_row << " pixel_col: " << pixel_col  << std::endl;
	    if(pixel_width_x<0.11&&pixel_width_y<0.151) {  // pixel 100x150 um^2
             psize = 0; 				    
	     if(xbin>xBinMax[psize]||ybin>yBinMax[psize]) {
//                std::cout << "psize: " << psize << "  xbin: " << xbin  << " ybin: " << ybin 
//                << " xBinMax: " << xBinMax[psize] << " yBinMax: " << yBinMax[psize] ;
//                std::cout  << " pixel_row: " << pixel_row << " pixel_col: " << pixel_col  << std::endl;
		continue;
	     }
	    }
